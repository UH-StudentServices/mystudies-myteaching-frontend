...
angular.module('directives.tour', ['services.userSettings', 'utils.browser', 'opintoniDialog'])
  ...
  .factory('TourTemplate', function($filter) {
    ...
    var startText = _.partial(translate, 'tour.common.start');
    var skipText = _.partial(translate, 'tour.common.skip');
    var nextText = _.partial(translate, 'tour.common.next');
    var finishText = _.partial(translate, 'tour.common.finish');

    function getElementTemplate(content, isEnd){
      var tpl = _.template('<div id="pop-over-text" class="popover-text"><%= content %></div><a class="skipBtn" aria-label="<%= skipText %>"></a><ul class="list-of-links--dark"><li><a class="nextBtn"><span><%= nextText %></span></a></li></ul></div></div>');
      return tpl({content : content, skipText : skipText(), nextText : isEnd? finishText() : nextText()});
    }

    function getTitleTemplate() {
      var tpl = _.template('<div><h3 class="popover-title">{{heading}}</h3><a class="skipBtn" aria-label="<%= skipText %>"></a><div class="popover-text" ng-bind-html="content"></div><ul class="list-of-links--dark"><li><a class="nextBtn"><span><%= startText %></span></a></li></ul></div>');
    ...
    return {
      ...
      link : function($scope) {
        ...
        function clickInsidePopover(event, popoverElement) {
          ...
          var width = popoverElement.width();
  ...
  .directive('studentTour', function(TourTemplate, $templateCache, UserSettingsService, TourElementSelectorByMedia) {
    ...
    return {
      ...
      link : function($scope) {
        ...
        _.each(_.filter(config, {'type' : 'element'}), function(e) {_.assign(e, commonElementProperties)});
  ...
  .directive('teacherTour', function(TourTemplate, $templateCache, UserSettingsService, TourElementSelectorByMedia) {
